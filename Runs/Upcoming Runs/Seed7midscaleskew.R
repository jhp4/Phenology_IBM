...
seed.reference <- 7
run.type <- "midscaleskew"
filetype <- ".csv"


# Set run seed

set.seed(seed.reference)
...
while(season < 71){ # Run for an initial 71 seasons (20 to burn in/stabilise, 50 to apply phenology shifts) 
  ...
  currentplantsummary <- plantinds %>% 
    ...
    summarise(population = length(speciesid),
              ...
              median.emergence = median(emergence),
...
write.csv(pollsummary,pollsummaryname, row.names = FALSE)
write.csv(plantsummary,plantsummaryname, row.names = FALSE)
write.csv(pollspeciesinfo, pollspeciesname, row.names = FALSE)
write.csv(plantspeciesinfo, plantspeciesname, row.names = FALSE)
write.csv(temp.summary, tempname, row.names = FALSE)
