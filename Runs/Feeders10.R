...
max_scale_sensitivity <-  0.02
min_scale_sensitivity <- -0.02
...
# Names of columns in pollinator species array (should be all traits that are species-specific. Currently 9 columns - species number, number of interaction partners, phenological model parameters (midpoint, scale, skew), parameter sensitivity (midpoint/scale), pollinator efficacy, generalist)
...
pollspeciesnames <- c("speciesid","interactions","midpoint","scale", "skew", "midpoint.sensitivity", "scale.sensitivity", "generalism", "lifespan", "efficacy", c(1:plant_species_number))


# Names of columns in plant species array (should be all traits that are species-specific. Currently 9 columns (although 1 unused) - species number, number of interaction partners, phenological model parameters (midpoint, scale, skew), parameter sensitivity). This should be kept equal to the number of named columns in the poll array for referencing of common functions. Currently this requires one 'blank' column at the end. 

plantspeciesnames <- c("speciesid","interactions","midpoint","scale", "skew", "midpoint.sensitivity", "scale.sensitivity", "generalism", "lifespan", "seedlings", c(1:poll_species_number))
...
make_poll_species<- function(poll_species_number    , 
                             ...
                             min_skew           ,
                             max_skew           ,
                             min_midpoint_sensitivity ,
                             max_midpoint_sensitivity ,
  ...
  poll_species[,6] <- runif( dim(poll_species)[1], min_midpoint_sensitivity, max_midpoint_sensitivity);
  poll_species[,7] <- runif( dim(poll_species)[1], min_scale_sensitivity, max_scale_sensitivity);
  poll_species[,8] <- rbinom(n = dim(poll_species)[1], size = 1, prob = generalism_likelihood);
  poll_species[,9] <- rpois(n = dim(poll_species)[1], lambda = poll_lifespan);
  poll_species[,10] <- runif( dim(poll_species)[1], min_efficacy, max_efficacy);
  return(poll_species)
}
...
pollspeciestraits <- make_poll_species( poll_species_number = poll_species_number    , 
                   ...
                   min_skew = min_skew           ,
                   max_skew = max_skew           ,
                   min_midpoint_sensitivity = min_midpoint_sensitivity ,
                   max_midpoint_sensitivity = max_midpoint_sensitivity ,
...
make_plant_species<- function(plant_species_number    , 
  ...
  plant_species[,6] <- runif( dim(plant_species)[1], min_midpoint_sensitivity, max_midpoint_sensitivity);
  plant_species[,7] <- runif( dim(plant_species)[1], min_scale_sensitivity, max_scale_sensitivity);
  plant_species[,8] <- rbinom(n = dim(plant_species)[1], size = 1, prob = generalism_likelihood);
  plant_species[,9] <- rpois(n = dim(plant_species)[1], lambda = plant_lifespan);
...
temp.change <- 0 # Temp change starts at 0 (as temperature changes parameters of binomial model that defines emergence will be redefined)
...
while(season < 11){ # Run for an initial 20 seasons 
...
write.csv(pollsummary,"pollsummaryfeeders10.csv", row.names = FALSE)
write.csv(plantsummary,"plantsummaryfeeders10.csv", row.names = FALSE)

pollsummary$speciesid<-as.factor(pollsummary$speciesid)
