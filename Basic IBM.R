...
## Here trying to collapse all the information relating to setting up the arrays
## for species and individuals into functions
...
#'Interactions' needs to match to the interactions column in the initial
#poll_species array (default is column 2). int_species needs to match the number
#of potential species that can be interacted with, which should equal the total
#number of plant species if running this function on the pollinator species info
#and vice-versa Number of partners is currently randomly sampled from 1:
#max(int_species) but again could make this more subtle if we want to directly
#tweak generalism/specialism rates among species
...
## Start up individuals array Species number = number of species to be added to
#the population (should be consistent with the number defined in the
#make_poll_species function) Cols = number of named columns (default is 8, will
#need to change if more traits added) Individuals = number of individuals of
#each species to be added to population (total number of individuals will always
#be individuals*species number). Currently set to add equal numbers of each
#species although this could be edited
...
# Define emergence function & initialise emergence #### Generates a random
# number between 0 and 1 for each individual and then back-calculates an
# emergence date from a logistic equation using the midpoint and scale values
# pulled from species information (for a full description of the logistic
# sampling see Logistic sample .Rmd file in project folder)
...
# Each individual will have an 'emergence' timestep as generated by the
# emergence function. At each timestep an individual's emergence will be checked
# against the current timestep. If equal then that individual becomes active and
# is first placed in the landscape then able to move (if pollinator). A 1
# signifies it's ready to be placed ( has emerged onto the landscape). A 2
# signifies it has already been placed but is still active (this stops the
# 'movement' function placing individuals on any timestep past emergence. An
# alternative would be to just not have individuals move when they first emerge)
